...
function MobileListItemComponent({
  ...
  return (
    <div
      ...
      className={`bg-white hover:bg-slate-50 border border-slate-200/80 rounded-xl p-4 flex items-center justify-between transition-all content-auto ${
        onClick
    ...
    >
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-semibold text-deep-space truncate leading-snug">
            {title}
          </h4>
          {subtitle && (
      ...
      <div className="flex items-center gap-2.5 ml-3 flex-shrink-0">
        ...
        {showArrow && (
          <RightOutlined className="text-xs text-slate-400 font-bold" />
        )}
      </div>
    </div>
  );
