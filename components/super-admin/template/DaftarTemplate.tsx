...
export function DaftarTemplate({ type, onRefresh, refreshTrigger }: DaftarTemplateProps) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const url = type === 'jenis-ujian' ? '/api/super-admin/jenis-ujian' : '/api/super-admin/template-raport';
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const url = type === 'jenis-ujian'
        ? `/api/super-admin/jenis-ujian?id=${id}`
        : `/api/super-admin/template-raport?id=${id}`;
