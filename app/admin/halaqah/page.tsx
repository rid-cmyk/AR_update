...
export default function AdminHalaqahPage() {
  ...
  const fetchSantri = async () => {
    try {
      ...
      const res = await fetch("/api/users?role=santri&excludeAssigned=true");
      ...
      <div style={{ padding: "24px 0" }}>
        ...
        >
          <Form form={form} layout="vertical" size="large">
            ...
            <Form.Item
              label="Pilih Santri (Maksimal 5 untuk awal)"
              name="santriIds"
              rules={[{ required: true, message: "Please select at least one santri" }]}
            >
              <Select
                mode="multiple"
