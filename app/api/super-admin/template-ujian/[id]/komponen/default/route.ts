...
export async function POST(
  ...
  try {
    ...
    const data = await TemplateUjianService.applyDefaultKomponen(user, parseInt(id), body.jenisUjian);
    return ApiResponse.success(data, 201);
  } catch (error: any) {
    if (error instanceof TemplateUjianServiceError) return ApiResponse.error(error.message, error.statusCode);
    return ApiResponse.serverError('Gagal membuat komponen default');
  }
}
