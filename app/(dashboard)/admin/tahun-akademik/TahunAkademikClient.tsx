...
import { SemesterDialog } from '@/components/admin/tahun-akademik/SemesterDialog'
...
export default function TahunAkademikClient({ initialTahunAkademik }: Props) {
  ...
  const handleDeleteTahun = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/admin/tahun-akademik/${id}`, { method: 'DELETE' })
      if (res.ok) {
  ...
  const handleSetActiveSemester = async (id: number) => {
    try {
      const res = await fetch(`/api/admin/semester/${id}/active`, { method: 'POST' })
      if (res.ok) {
