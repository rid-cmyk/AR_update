...
interface TahunAkademikClientProps {
  initialTahunAkademik: TahunAkademik[]
}

export default function TahunAkademikClient({ initialTahunAkademik }: TahunAkademikClientProps) {
  const [tahunAkademik, setTahunAkademik] = useState<TahunAkademik[]>(initialTahunAkademik)
  const [loading, setLoading] = useState(false)
  const [selectedTahun, setSelectedTahun] = useState<TahunAkademik | null>(null)
  const [showDialog, setShowDialog] = useState(false)
  const [showAutoGenerate, setShowAutoGenerate] = useState(false)
  
  // Hitung activeTahunAkademik dari data
  const activeTahunAkademik = tahunAkademik.find(ta => ta.isActive) || null;
  ...
  if (loading && tahunAkademik.length === 0) {
    return (
      <>
    ...
    <>
      <div className="space-y-6" style={{ padding: '0 4px' }}>
        <AdminHeaderCard
          ...
          subtitle="Kelola pengaturan tahun akademik dan semester otomatis"
          actions={
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {activeTahunAkademik ? getSemesterIcon(activeTahunAkademik.semester) : '📅'}
                  </div>
                  <p className="text-xs text-muted-foreground">
