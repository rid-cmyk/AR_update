...
import { TemplateUjianDialog } from '@/components/super-admin/template-ujian/TemplateUjianDialog'
import { KomponenPenilaianDialog } from '@/components/super-admin/template-ujian/KomponenPenilaianDialog'
import TemplateUjianCard from '@/components/super-admin/template-ujian/TemplateUjianCard'
import { useToast } from '@/hooks/use-toast'
import AdminHeaderCard from '@/components/super-admin/layout/AdminHeaderCard'
import { TemplateUjian } from '@/components/super-admin/template-ujian/templateUjianTypes'
...
export default function TemplateUjianClient({ initialTemplates }: TemplateUjianClientProps) {
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/super-admin/template-ujian')
      if (response.ok) {
  ...
  const handleDeleteTemplate = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/super-admin/template-ujian/${id}`, {
        method: 'DELETE'
      })
  ...
  const handleToggleStatus = async (id: number, isActive: boolean) => {
    try {
      const response = await fetch(`/api/super-admin/template-ujian/${id}/toggle`, {
        method: 'PATCH',
