...
import { TemplateRaportDialog } from '@/components/super-admin/template-raport/TemplateRaportDialog'
import { PreviewRaportDialog } from '@/components/super-admin/template-raport/PreviewRaportDialog'
...
interface TemplateRaportClientProps {
  initialTemplates: TemplateRaport[]
}

export default function TemplateRaportClient({ initialTemplates }: TemplateRaportClientProps) {
  const [templates, setTemplates] = useState<TemplateRaport[]>(initialTemplates)
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/super-admin/template-raport')
      if (response.ok) {
  ...
  const handleDeleteTemplate = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/super-admin/template-raport/${id}`, {
        method: 'DELETE'
      })
  ...
  const handleToggleStatus = async (id: number, isActive: boolean) => {
    try {
      const response = await fetch(`/api/super-admin/template-raport/${id}/toggle`, {
        method: 'PATCH',
  ...
  const handleExportTemplate = async (id: number) => {
    try {
      const response = await fetch(`/api/super-admin/template-raport/${id}/export`)
      if (response.ok) {
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    <Badge variant="outline">
                      {template._count?.raportSantri || 0} raport
                    </Badge>
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={(template._count?.raportSantri || 0) > 0}
                  >
