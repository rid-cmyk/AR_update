...
import GuruPermissionsFormModal from "@/components/super-admin/guru-permissions/GuruPermissionsFormModal";
...
export default function GuruPermissionsClient({ 
  ...
  const fetchPermissions = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/super-admin/guru-permissions");
  ...
  const fetchGurus = useCallback(async () => {
    try {
      const res = await fetch("/api/super-admin/users?role=guru");
  ...
  const handleSave = async () => {
    try {
      ...
      const url = editingPermission 
        ? `/api/super-admin/guru-permissions/${editingPermission.id}` 
        : "/api/super-admin/guru-permissions";
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await fetch(`/api/super-admin/guru-permissions/${id}`, { 
        method: "DELETE" 
      });
